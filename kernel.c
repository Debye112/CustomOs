...
//stringcom() = if strigs are equal returns 1 if not retuen -1.
...
void disk_read(int block, void *buffer);
void disk_write(int block, void *buffer);
...
void loadNodes() {
    disk_read(NODE_BLOCK, nodePool);
}

void saveNodes() {
    disk_write(NODE_BLOCK, nodePool);
}
